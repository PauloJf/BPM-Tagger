...
- **Run from a playlist** — build the run queue from your **whole library** or a specific **playlist** via a source picker; the pool is scoped to that playlist's matched, BPM-tagged tracks, and a per-playlist "N of M available" count shows how much of it is actually runnable. The run **stays inside the chosen playlist as it auto-refills** — reshuffling its tracks once the unplayed matches run out, rather than drifting back to the whole library
