...
╚═════╝ ╚═╝     ╚═╝     ╚═╝     ╚═╝   ╚═╝  ╚═╝ ╚═════╝  ╚═════╝╚══════╝╚═╝  ╚═╝
                       ▁ ▂ ▃ ▅ ▇ █ ▇ ▅ ▃ ▂ ▁ ▂ ▃ ▅ ▇ ▅ ▃ ▂ ▁
             cadence-synced running music from your own library
...
- **[The BPM engine](#the-bpm-engine)** — three-detector BPM analysis, tag writing, review queue, locks. The original core; everything else stands on it.
- **[Run — the cadence player](#run--the-cadence-player)** — pick a step cadence and run to your own library: octave-folded queue building weighted by your 1–5 star ratings, a pitch-preserving tempo lock, offline preloading for network dead zones, and a locked-down kiosk mode for handing someone a phone. **Listen**, the regular non-cadence player, covers everything that isn't a run.
- **[The library companion](#the-library-companion)** — playlists from Spotify, Navidrome, or built by hand; lyrics; cover & artist art; duplicate resolution; ISRCs; and two-way star sync, scrobbling, and auto-rescan back into Navidrome.
- **[The music grabber](#music-grabber-optional-grabber_enabledtrue)** *(optional, off by default)* — watch your own Spotify playlists, see what's missing from disk, and download it — **Deezer** (your own ARL) with a **yt-dlp** fallback — transcoded to one format, fully tagged, BPM-analyzed, and filed by a path template, with an ambiguity inbox and [ntfy](https://ntfy.sh/) pings.

It was built by a runner who needed BPM-tagged music for cadence-synced running — which is why the tagger came first, and the player is what it became.
...
The part the name promises: detect every track's tempo, write it to the tag, and flag the doubtful ones for a human.
...
### Run — the cadence player

The reason the engine exists: a tempo-run player over your own library, built on the tags above.
...
- **Run journal & per-account stats** — every tempo run is also recorded as a journal entry (date, duration, source, tracks, average cadence, % of time tempo-shifted) attributed to the account that ran it — admin, the shared Guest login, or a named player user. The Stats page lists recent runs and lets you filter the cumulative Run-mode totals per account; history starts at the upgrade, so older listening stays in the all-time totals (shown as *(unattributed)* where material)
- **Ratings & dislikes** — rate tracks **1–5 stars** from the player bar, track page, library, Run queue or the Run journal, and dislike the ones you never want offered. Both are **per account**: each player user has their own ratings, and one user's dislike never hides a song from anyone else. Ratings **weight** how often a track is picked by Run, Listen shuffle and radio, and "similar" (a 5★ track turns up about 6× as often as an unrated one by default, a 1★ rarely), tunable in **Settings → Ratings & picking**; a disliked track is never picked or shuffled in, whatever its rating (playing its album or playlist in order still includes it). A rating of 4★ or more counts as **starred** (for Navidrome and Subsonic apps). Library filters for **Unrated**, **Rated ≥ N**, **Starred** and **Disliked**, plus a **Rating** sort
- **Queue similar from the player** — a similar-tracks button on the player bar (and **≈ Similar** on the Run page's queue view) opens two lists:
  - **From your library**: offline picks that are always playable. First the same artist (every credited artist), then tracks at a similar tempo (±5 % of the song's BPM, with half and double tempo counting). During a tempo-locked run, the tempo band follows the run's cadence and stretch limit instead, so every pick fits the run. Each row says why it was picked.
  - **From Deezer**: the artist's Deezer radio. In-library matches queue straight onto the **play queue** (cadence-checked during a run; tracks that can't stretch onto the target show **off cadence**), and missing ones get a **Grab** button for the download queue.
  - **Queue all** adds every eligible match from both lists at once.
...
- **Floating mini player** — pop the now-playing card out into an always-on-top window that stays put as you switch tabs or apps: the track's **cover art fills the background** (blurred, behind a glass now-playing card), title/artist with a **beat-paced equalizer** beside the title (its bars bounce in time with what you hear — the locked cadence, or the track's native BPM), a seekable progress bar, prev/play/next, volume, and (in Run mode) the tempo-lock pill with the pulsing beat dot. Uses the Document Picture-in-Picture API (Chromium desktop); **clicking the Run page's cover pops it out** (the button also sits in the player bar), it's hidden where the API isn't available, and opening it as an installed PWA tucks the main window away where the runtime allows. Playback stays in the main tab — the window just drives it — and it follows the app's light/dark theme
- **Listen mode** — the regular, non-cadence player (`/listen`): play the whole library or any playlist, in order or shuffled, at native speed — no BPM required — with a **radio** that keeps the queue going, either from the same source or with **similar** tracks from your library. See [Listen](#listen-listen)
- **Player mode (kiosk)** — a second, locked-down login for handing someone a phone or dedicating a device to running: the shared **Guest login** (`RUN_PASSWORD`) or **named player users** scoped to chosen playlists, confined server-side (default-deny) to the player endpoints. A four-level setting decides whether the kiosk gets Listen next to Run — up to a pure jukebox — and each named user can override it individually (inherit / off / on / default / only). See [Player mode](#web-ui)
- **Installable PWA with offline audio** — add to the home screen, control from the lock screen (Media Session), and ride out network dead zones: the player fully downloads the next few queue tracks ahead of the playhead, and per-preset **Prepare offline** buttons pin a whole run's queue into a capped per-device cache. See [Offline preloading](#offline-preloading)
...
- **Play counts in the library** — a sortable **Plays** column in the Tracks table (most-played first, toggling back to the default order) and play counts on playlist rows; **Play all** queues exactly the sort you see
...
- **Navidrome star sync** — two-way: stars set in BPM Tagger push to Navidrome as favourites, stars set in Navidrome pull in. A star is a rating of 4★ or more: a star pulled from Navidrome rates an unrated or lower track 4★, an un-star drops a 4–5★ track to 3★. A per-track baseline keeps "starred here" and "un-starred there" apart; **Sync stars now** lives in Settings → Navidrome
...
**Playlists are first-class** — from Spotify, Navidrome, or built by hand, and any of them can be a Run source:

- **Playlist stats strip** — every playlist detail page opens with its matched tracks' total runtime, a mini BPM histogram, total plays with the top played tracks, per-preset runnable counts (computed by the exact rule the run queue uses — the strip and the playlist cards can never disagree), and when the playlist last changed
- **Compare, merge & split** — **diff** two playlists into *both / only in A / only in B* (tracks matched by library file, ISRC, or normalized artist+title, so the same song in two different files still lands in *both*), **merge** several playlists into a Local one (deduped by the same identity chain, with an added / already there / not in library report), or **split** a playlist into Local playlists **by cadence** (one per run preset, using the exact run-queue eligibility rule) or **by artist** (groups of 3+). Every output is a Local playlist — these operations never write back to Spotify or Navidrome
...
| `NAVIDROME_SYNC_RATINGS` | `false` | Initial state of the **two-way rating sync** toggle (Settings → Navidrome): the admin's 1–5 star ratings ⇄ Navidrome's ratings for the configured user. While it's on, star sync only pushes (Navidrome's stars mirror your ratings). |
...
| `SUBSONIC_RUN_PLAYLISTS` | `true` | Show each Run preset as a read-only **Run · <name> (<bpm> BPM)** playlist: tracks within 4 % of the preset's BPM (half and double time included), weighted by the listening account's ratings and skipping its dislikes. Subsonic apps can't tempo-lock, so these play at native speed, hence the tight band |
...
**Accounts.** The admin sees the whole library and can create and edit Local playlists. A **player user** has no Subsonic access until you generate credentials for it, and then sees only the tracks of the playlists it's associated with (the same rule as Run mode). It can rate (`setRating`), star and scrobble those tracks, but not edit playlists. Ratings are per account in Subsonic apps too: each account sees its own `userRating`, a star is a rating of 4★ or more, and random songs, similar songs and the Run playlists are weighted by the caller's ratings and skip its dislikes. Disabling or deleting the player user cuts its Subsonic access off on the next request.
...
| `UI_USERNAME` | _(empty)_ | Optional **username for the admin login**. Empty keeps the historical password-only sign-in (leave the login form's username field blank). Set one and you can sign in with username + password — which is what password managers expect. A blank username **always keeps working** for the admin, so this can never lock you out. Also settable at runtime in **Settings → Admin sign-in**. |
...
| `PICK_USE_RATINGS` | `true` | Weight Run, Listen shuffle/radio, similar and Subsonic picks by each account's ratings (Settings → Ratings & picking). `false` = every non-disliked track equally likely |
| `PICK_WEIGHTS` | `0.1,0.5,1,1,3,6` | Relative pick weights, in order **1★, 2★, 3★, unrated, 4★, 5★**. `0` = only when nothing else fits |
| `PICK_NEW_FACTOR` | `1` | Multiplier on **new** tracks (unrated *and* never played by that account): `0` never (unless nothing else fits), `0.5` less, `1` neutral, `2` more, `4` much more |
| ~~`RUN_PREFER_STARRED`~~ | — | **Deprecated** (replaced by ratings): `false` still turns `PICK_USE_RATINGS` off for one release, with a log warning |
| ~~`RUN_PREFER_FAMILIAR`~~ | — | **Deprecated**: `true` still sets `PICK_NEW_FACTOR` to `0.5` ("less") for one release, with a log warning |
...
| `RUN_PRELOAD_TRACKS` | `10` | How many tracks a per-preset **Prepare offline** download grabs on the Run page (see [Offline preloading](#offline-preloading)) |

### Offline preloading defaults

Editable at runtime in **Settings → Playback**; the env vars only set the initial values. Requires the UI to be served over **HTTPS** (or `localhost`) — the browser's Cache API doesn't exist on plain-HTTP origins, in which case the player simply streams as before. See [Offline preloading](#offline-preloading) under Web UI.

| Variable | Default | Description |
|---|---|---|
| `PRELOAD_AHEAD` | `5` | How many upcoming queue tracks the player fully downloads while playing, so the music rides out network dead zones. `0` disables the look-ahead |
| `PRELOAD_CACHE_MB` | `500` | Per-device cap on cached audio; past it the tracks untouched longest are evicted first |
...
- **The app shell and API are never cached** — by design, so the UI and its data can never go stale. The one thing that *is* cached is audio you preload: the player's look-ahead and the Run page's **Prepare offline** downloads store whole tracks on the device so playback rides out network dead zones — see [Offline preloading](#offline-preloading). Opening the app still needs connectivity to your server.

#### On a desktop (Windows / macOS / Linux)

The same install works on a computer — there is no separate download, and none is needed. In **Chrome or Edge**, open the UI and either click the install icon in the address bar or use ⋮ → *Cast, save and share* → *Install page as app*. You get a standalone window with no browser chrome, an entry in the Start menu / Dock / Launchpad, its own taskbar icon, and the same Media Session controls — so the media keys on your keyboard drive playback. Safari on macOS does the same via *File → Add to Dock*. It still needs HTTPS, exactly as on a phone.

**There is deliberately no Electron or Tauri build.** BPM Tagger is a server you already run; a desktop shell would be a window pointing at it, offering essentially what the installed PWA already offers. What it would add — a tray icon, global hotkeys beyond the media keys — does not pay for what it costs to keep honest: code signing on two platforms, an auto-update channel, and a three-OS build matrix shipping alongside the Docker images. If you want the app pinned to your taskbar, install it from the browser and you are done.

### Run mode (`/run`)

A full-screen tempo-run player, like the cadence apps — but drawing on your own library and your own BPM tags. Top to bottom: the playing track's **cover art** (on phones it flexes to exactly the leftover screen height — the page never scrolls, like a native player, whether in a browser or the installed PWA), its **title and artist** (on desktop with the track's **play count** just below) — the title links to the track page, so a wrong BPM can be fixed mid-run — the big **target BPM** readout with the **tempo-lock toggle** beside it, then a line showing exactly what you're hearing — `NATIVE 78 · 0.99× ×2 → 155 BPM` (native BPM, stretch factor, octave multiplier, resulting cadence) with a dot pulsing on your step. Below that, a **waveform** with elapsed/remaining time, big **play / prev / next** buttons and a **lyrics** drawer. The global player bar stays hidden on this page — the run player *is* the transport, and connection trouble surfaces right on the page: a **Buffering · N%** indicator when a stream stalls and a clear **Offline** state when the connection drops, shown as a note floating just above the waveform (one at a time, together with queue notices like a stale target or a source change), so a slow link never looks like an app fault and a note can never push the controls off a phone screen. To fight the stalls, an **adaptive rebuffer hold** waits for a growing amount of buffered-ahead before resuming after an underrun (more on each repeated stall), turning constant stutter into a few clean pauses; and the player **fully downloads the next few tracks** ahead of the playhead so they play from the device with no track-boundary stalls — see [Offline preloading](#offline-preloading).
...
2. **Start run** — builds a queue of every track that can reach the target within your **max stretch** limit (default ±15%; one setting, in Settings → Run Mode). Anything inside the limit is eligible, and the queue is drawn from them **weighted by your ratings**. With **octave matching** on (default), half- and double-time tracks count too: at a 150 cadence a 75 BPM song plays untouched and you step on every beat. Higher-rated tracks turn up more often and 1★ ones rarely; **your disliked tracks are never picked at all** (toggle the dislike on a queue row, the track page, or the library — reversible any time). Rate tracks straight from the queue. When the **last queued track starts playing, a fresh batch for the same target is fetched and appended automatically**, favoring tracks not already in the queue — the music never runs out mid-run (with repeat on, the queue loops instead).
...
The regular music player, for everything that isn't a tempo run. It's a full-screen now-playing view over the same playback engine as everywhere else: big **cover art** (with the ambient glow), **title / artist** (linked to their pages on the admin view), the track's **BPM** pulsing on the beat, a **seekable waveform** with elapsed/remaining time, big **play / prev / next** buttons flanked by **star** and **dislike**, and a row with **shuffle / repeat / radio / lyrics** (+ volume on desktop). The global player bar hides here (this page *is* the transport), and connection trouble (offline / buffering) surfaces right above the waveform, just like on Run. On desktop the page uses the same **two-column layout** as Run's cockpit: the player owns the left column — the cover grows with the window height — and the **queue** (the same drag-to-reorder list as the player bar's drawer) lives in a full-height panel beside it, ending level with the transport, with the source picker on board so switching sources never covers the artwork. On phones the page fits **one screen without scrolling**, like Run: the cover flexes to the leftover height, the transport pins low, and a **bottom Playing / Queue switcher** swaps the cover for the queue (where the source picker lives mid-playback).
...
- **Radio** — keeps the queue going as it nears its end, the non-cadence sibling of Run's auto-refill. With radio on, pick what it refills from:
  - **Source**: more of the source you started (recent tracks avoided, recycling once a small pool is exhausted, weighted by your ratings, your disliked tracks never picked). **Shuffle** on a playlist or the whole library is rating-weighted too (favourites tend to come earlier, everything still plays); an album or artist shuffle stays a plain shuffle, minus your dislikes.
  - **Similar**: tracks like the one playing, from your library: the same artist first, then a similar tempo. It needs no source, so it keeps any queue going (an album, a search result, one track). Each batch starts from the queue's last track, so the music drifts gradually instead of looping one artist. It's offline and never picks disliked tracks.
...
Built for places where the network dies mid-run — a trail with dead zones, a basement gym, a phone that drops Wi-Fi at the door. The player's `<audio>` element always streams the same `/audio` URL; a service worker serves those URLs from a **per-device audio cache** whenever the track was preloaded, and falls through to the network otherwise. Playback is byte-for-byte identical either way — being cached just means the network is no longer needed for that track.

Two things fill the cache:

- **Automatic look-ahead** — while a queue is playing (Run *or* Listen), the next few tracks (default 5, **Settings → Playback → Preload ahead**, `0` = off) download in full at low priority. Short outages and track boundaries stop mattering: whatever is in the window keeps playing.
- **Prepare offline** (Run page) — a row of per-preset chips under the BPM presets (on phones they show before playback starts, keeping the one-screen mid-run layout intact; desktop shows them always). Tapping one builds a queue for that preset's cadence and source, shows the download size (e.g. `Steady · 82 MB?`), and a second tap downloads it (default 10 tracks, **Settings → Run Mode → Prepare offline size**). Because queue builds are randomized, the exact queue is **pinned** with the download — if you hit **Start run** at that preset's BPM while the server is unreachable, the pinned queue starts and plays entirely from the cache. A ✓ on the chip means every pinned track is cached; tap again anytime for a fresh queue.

Housekeeping is automatic: the cache is capped (default 500 MB, **Settings → Playback → Offline cache cap**) and the tracks untouched longest are evicted first; **Settings → Playback → This device's cache** shows the current usage with a **Clear** button. Notes: preloading needs **HTTPS** (or `localhost`) — on plain-HTTP origins the Cache API doesn't exist and the player streams exactly as before; the cache is **per device** (prepare on the phone you run with); and signing in is still needed to *open* the app — preloading keeps music playing through dead zones, it doesn't make the whole app offline.
...
- **Navidrome integration** — URL, username, and password for auto-rescan (with a **Test** button), plus the **two-way star sync** and **two-way rating sync** toggles with their **Sync stars now** / **Sync ratings now** actions
- **Playback** — seconds of audio to buffer before the detail-page player starts
- **Ratings & picking** (admin) — whether ratings weight the picks, the six relative weights (1★ … 5★ and unrated), and the **new songs** setting (Never · Less · Neutral · More · Much more, or a custom multiplier), with a live preview of the share of picks each rating level would get from your library. Weights are global; ratings are per account. The Run stretch limit and dislikes stay hard rules
- **Run Mode** — the four named BPM presets (name + value each), octave matching, queue size, and **max stretch %** — the single limit that decides which tracks a run queues and how far they're stretched
...
- **Run mode** _(after your first run)_ — cumulative tempo-run totals: tracks played, total time on feet (with native audio duration covered), the share of time tempo-shifted vs at native speed, average cadence, and a time-per-cadence breakdown — now with an **account filter** (All / Admin / Guest / each named player user; runs recorded before this version have no account and show under All, with a visible *(unattributed)* bucket where material)
- **Run journal** _(after your first run on this version)_ — recent tempo runs, 15 at a time with Show more: date, account, duration, source (library or playlist), tracks played, average cadence, and % of time tempo-shifted; live runs are marked while they're still open
...
- **Optional admin username** — the admin account can be *named* (**Settings → Admin sign-in**, or `UI_USERNAME`) so password managers have a user field to save; it's an identifier, not a second secret. Signing in with a blank username keeps working, so setting one can't lock you out, and it can't collide with a named player user in either direction.
- **Admin two-factor (TOTP)** — the admin login can require a time-based authenticator code (RFC 6238) in addition to the password. Enrol under **Settings → Two-factor** (scan or type the secret, confirm one code) and save the one-time **recovery codes** shown once. The TOTP secret and recovery-code hashes are stored in `settings.json` at the same trust level as the password hash. A lost authenticator is recoverable with a recovery code, or — if those are gone too — by running the app once with `MODE=disable_2fa`. Implemented with the standard library (no added dependency). Two-factor applies to the admin only. The code field lives in the same form as the username and password (rather than replacing them), so a password manager can autofill the one-time code as part of the login.
...
- Optionally set `UI_USERNAME` (or **Settings → Admin sign-in**) so your password manager can store a username alongside the password. The password still does the authenticating.
