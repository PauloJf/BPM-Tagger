...
- **Floating mini player** — pop the now-playing card out into an always-on-top window that stays put as you switch tabs or apps: cover, title/artist, a seekable progress bar, prev/play/next, volume, and (in Run mode) the tempo-lock pill with the pulsing beat dot. Uses the Document Picture-in-Picture API (Chromium desktop); the pop-out button sits in the player bar and on the Run page's cover, and is hidden where the API isn't available. Playback stays in the main tab — the window just drives it — and it follows the app's light/dark theme
