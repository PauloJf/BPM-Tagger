...
- **[Run — the cadence player](#run--the-cadence-player)** — pick a step cadence and run to your own library: octave-folded queue building that prefers your starred tracks, a pitch-preserving tempo lock, offline preloading for network dead zones, and a locked-down kiosk mode for handing someone a phone. **Listen**, the regular non-cadence player, covers everything that isn't a run.
...
- **Radio** — keeps the queue going as it nears its end, the non-cadence sibling of Run's auto-refill. With radio on, pick what it refills from:
  - **Source**: more of the source you started (recent tracks avoided, recycling once a small pool is exhausted, disliked tracks never picked).
