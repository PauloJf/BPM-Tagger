...
**v2.6.5** · [Changelog](CHANGELOG.md) · [![Docker Pulls](https://img.shields.io/docker/pulls/gatoserio/bpm-tagger)](https://hub.docker.com/r/gatoserio/bpm-tagger)
...
Same sign-in screen gates the full admin UI and the locked-down **player mode** (the shared `RUN_PASSWORD`, or a **named run user** with an optional username — Settings → Player Access) — the difference shows up after login, in what the session can reach:
...
- **Local playlists + "Add to playlist"** — build a playlist by hand: create one by name, then add any library track from an **"Add to playlist"** button on its track page or a Library row (pick an existing local playlist or create one inline). Added tracks are on-disk by definition, so they count as **have** immediately — no sync, no missing tracks; remove a track or delete the playlist whenever you like. Local playlists need no grabber at all
...
- **Queue** — live download progress, retry/cancel, delete failed items, clear completed, and history
...
| `RUN_PASSWORD` | _(empty)_ | Optional **shared player password** that logs into a locked-down **player mode** showing only the Run page (plus a runner-focused About) — the tempo player, nothing else. Hand a phone or tablet to someone for a run without exposing your library, settings, or downloads. This shared login is **full-access** (all playlists + the library/starred pools). Leave empty to keep the single admin login. For per-person accounts with playlist scoping, create **named run users** in **Settings → Player Access → Run users** (see [Player mode](#web-ui)). Also settable at runtime in **Settings → Player Access**. |
| `RUN_SESSION_DAYS` | `30` | How long a **player** login stays signed in, in days (sliding). Applies to the shared password and named run users alike. A running kiosk is low-privilege and opened repeatedly, so it defaults far longer than the admin session. Also settable in **Settings → Player Access**. |
| `RUN_FORCE_TEMPO` | `false` | Default for the Run page's **"play everything, force tempo"** toggle: ignore the BPM match tolerance and force every track onto the target cadence (extreme rates clamped). Per-run overridable on the Run page; also settable in **Settings → Run Mode**. |
...
**Named run users (optional):** beyond the shared `RUN_PASSWORD`, create **individual run accounts** under **Settings → Player Access → Run users**, each with its own username + password. Sign in with the optional **username** field on the login screen (leave it blank for the admin password or the shared guest). A user is either **full-access** (whole library, starred, and every playlist as run sources) or **restricted** to the playlists you check for them — a restricted user's Run source picker only offers their playlists, and the server refuses the library/starred pool and any other playlist. Reset a password, disable, or delete a user and their sessions end at once. Accounts live in BPM Tagger's own database (no Navidrome credentials are involved); the existing `RUN_PASSWORD` keeps working unchanged as a shared, full-access guest, so nothing changes on upgrade. Playlist scoping is a curation boundary, not a lock on the audio — media streaming stays path-validated as before.
...
2. **Start run** — builds a queue of tracks whose BPM matches the target within a tolerance. With **octave matching** on (default), half- and double-time tracks count too: at a 150 cadence a 75 BPM song plays untouched and you step on every beat. **Starred tracks are picked first**; **disliked tracks are never picked at all** (toggle the dislike on a queue row, the track page, or the library — reversible any time). When the **last queued track starts playing, a fresh batch for the same target is fetched and appended automatically**, favoring tracks not already in the queue — the music never runs out mid-run (with repeat on, the queue loops instead).
3. **BPM lock** — the lock button beside the target stretches every queued track onto the exact target using the browser's pitch-preserving time-stretcher (`playbackRate`). Only the small post-fold remainder is stretched (capped by the **max stretch** setting, default ±15%), so tracks keep sounding natural. Changing the target mid-run re-stretches the current track instantly — and so does **saving a corrected BPM** on the track page; the lock follows the fixed value without a rebuild. On other pages, the player bar shows a lock icon with the target BPM, pulsing on your step.

Unlock to keep the BPM-matched queue but hear every track at native speed.

**Play everything (force tempo).** A toggle on the Run page (default set in Settings → Run Mode) drops the BPM match tolerance and forces **every** track onto your target cadence via the tempo lock — so any track can fill the queue, not just those already near the target. Tracks too far off even after octave folding have their playback rate clamped (and are flagged as not exactly on target) so nothing turns into a chipmunk or a rumble.
