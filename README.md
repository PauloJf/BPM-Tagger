...
**v2.11.1** · [Changelog](CHANGELOG.md) · [![Docker Pulls](https://img.shields.io/docker/pulls/gatoserio/bpm-tagger)](https://hub.docker.com/r/gatoserio/bpm-tagger)
...
The regular music player, for everything that isn't a tempo run. It's a full-screen now-playing view over the same playback engine as everywhere else: big **cover art** (with the ambient glow), **title / artist** (linked to their pages on the admin view), the track's **BPM** pulsing on the beat, a **seekable waveform** with elapsed/remaining time, big **play / prev / next** buttons flanked by **star** and **dislike**, and a row with **shuffle / repeat / radio / lyrics / volume**. Below it, the **queue** — the same drag-to-reorder list as the player bar's drawer. The global player bar hides here (this page *is* the transport), and connection trouble (offline / buffering) surfaces right above the waveform, just like on Run.

- **Play a playlist** — pick one in the source picker and hit **Play** (playlist order) or **Shuffle**. Unlike a run queue, **no BPM is required**: un-analyzed tracks play too, at native speed, exactly as filed. Admins can equally start playback from any album, artist, playlist or library page and use Listen purely as the now-playing screen.
- **Radio** — with a playlist playing, the radio toggle keeps the queue topped up from the same playlist as it nears its end (recent tracks avoided, recycling once a small playlist is exhausted, disliked tracks never picked) — the non-cadence sibling of Run's auto-refill.
