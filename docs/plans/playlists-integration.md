...
Status: **Phase 1 implemented** (2026-07-17) — schema generalization, diff/upsert
sync, and the legacy migration are in `db.py` / `grabber/sync_engine.py` /
`web/api/playlists.py`, with `tests/test_playlist_sync.py`. Phases 2–5 proposed.
...
| **2** | **Navidrome source**: Subsonic `getPlaylists`/`getPlaylist`, add-by-pick, sync+resolve+coverage, new-track tracking UI. |
