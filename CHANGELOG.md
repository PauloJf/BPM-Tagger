...
- **Delete failed queue items.** Failed (and skipped) tracks in the download queue now have a **Delete** button beside Retry, so you can clear individual dead entries instead of only retrying them. It removes the queue record and its history in one go; a file that already downloaded and filed into the library is untouched. (In-flight items still use Cancel.)
