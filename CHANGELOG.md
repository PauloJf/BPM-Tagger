...
- **Slow-connection visibility.** Playback now surfaces network trouble instead of looking like an app bug: a **Buffering · N%** indicator (with a spinner) shows when a stream stalls waiting for data, and a clear **Offline — waiting for connection** state appears when the browser reports the connection dropped — on both the player bar and the Run page (where the global player bar is hidden). In **Run mode**, the next couple of tracks are now **pre-buffered** in the background (the queue is predictable there), so a track change is far less likely to stall on a slow link.
