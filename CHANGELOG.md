...
- **More hardening.** The session cookie can be forced `Secure` for a TLS-terminating proxy via `UI_FORCE_SECURE_COOKIE` (it was already automatic for an `https://` `UI_PUBLIC_URL`); a too-short plaintext `UI_PASSWORD` is flagged at startup; `/healthz` library stats are now shown to an admin only (never a player kiosk); a top-level **error boundary** keeps a single page crash from white-screening the whole app; the grabber's per-item **audit-event log is now capped** (with a one-time cleanup) so it can't grow without bound on a long-lived queue item; and CI now runs **dependency vulnerability scans** (`npm audit` on production deps, advisory `pip-audit`).
