...
- **Player kiosk UI:** since player mode hides the sidebar, the Run page shows its own compact **brand banner** (logo + "Run mode" note, with sign-out) in that mode. On phones the player also gets a **one-screen layout** — the waveform and transport controls are pinned to the bottom while the cockpit above (cover, target, presets) distributes the remaining space, so nothing scrolls off.
