...
- **Run mode** — a full-screen tempo-run player that fits one phone screen: viewport-scaled cover art, big target-BPM readout with the tempo-lock toggle and a `native · stretch × octave → result` breakdown, ±1/±5 steps, four **named presets** or an in-place **queue view**, lyrics drawer, waveform + large transport; auto-queues octave-folded BPM matches (**starred tracks** first), **refills the queue automatically when the last track starts**, and **locks the tempo** so every song stretches onto your step, pitch preserved
