...
- **Lyrics** — plain or synced (LRC) lyrics from LRCLIB per track or in bulk, viewable/editable on the track page, stored embedded or as `.lrc` sidecars (Navidrome-compatible); a **player lyrics drawer** follows synced lyrics live (click a line to seek), steps plain lyrics manually, has an **S / M / L** text-size stepper, and can be **maximized or drag-resized**
...
- Persistent player bar with **Play all / Shuffle** queueing (labelled by tag title, not filename), prev/next, repeat, volume, a **queue viewer** showing cover art + BPM per row with **drag-to-reorder** (↑/↓ too), **Clear**, and **maximize / drag-resize**, a reload-persistent queue that resumes at the saved position, keyboard shortcuts (`k` play/pause, `q` queue, `l` lyrics, arrows, volume, mute), and a ducking **preview** from detail/compare views
...
**v2.7.3** — **Custom accent color.** The interface's purple accent became yours to change under **Settings → Appearance** — a row of accent swatches or a free hue slider, recoloring the whole UI at once (logo, buttons, badges, focus rings, sliders, progress bars, and the login screen's glow + animated bars) and staying balanced in both themes, applied before the first paint so there's no flash. (v2.7.4 makes the choice per-account.)
