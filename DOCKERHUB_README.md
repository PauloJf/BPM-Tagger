...
| `PLAYER_LISTEN_MODE` | `off` | What player logins get besides Run mode: `off` Run-only kiosk · `on` adds a **Listen** tab (regular playlist player) · `default` Listen is also the landing page · `only` pure jukebox (no Run). Named player users can carry a **per-user override** (unset = inherit this). Also settable in Settings → Player access |
...
- **Artists & Albums browse views** — a Tracks | Artists | Albums switcher with filterable card grids (track counts, years, average BPM) linking into the per-artist/per-album pages; the Tracks table carries a sortable **Plays** column (local play counts). Every browse page shares the same **Play / Shuffle / Add to queue** trio — Add to queue appends without replacing what's playing — and each row's queue button reflects whether that track is actually queued
- **Playlist stats & operations** — every playlist detail opens with a stats strip (matched runtime, mini BPM histogram, plays + top played, per-preset runnable counts computed by the exact run-queue rule, last change); **Compare** two playlists (tracks matched by file / ISRC / normalized artist+title), **Merge** several into a Local playlist, or **Split** one by cadence (one Local playlist per run preset) or by artist — all outputs are Local playlists, nothing ever writes back to Spotify or Navidrome
- **Run journal & per-account run stats** — every tempo run is journaled (date, account, duration, source, tracks, average cadence, % tempo-shifted) and attributed to admin / the shared Guest / the named player user that ran it; the Stats page lists recent runs and filters the cumulative Run totals per account (pre-upgrade history stays in the all-time totals)
...
- Persistent player bar with **Play all / Shuffle** queueing (labelled by tag title, not filename), prev/next, repeat, volume, a **queue viewer** showing cover art + BPM per row with **drag-to-reorder** (↑/↓ too), **Clear**, an **S/M/L/XL text-size stepper**, and **maximize / drag-resize**, a reload-persistent queue that resumes at the saved position **and follows your account across devices** (server-side per-account snapshot — admin and named player users; the shared Guest login stays per-browser), keyboard shortcuts (`k` play/pause, `q` queue, `l` lyrics, arrows, volume, mute), and a ducking **preview** from detail/compare views
...
- **Player mode** _(optional)_ — a locked-down view showing **only the Run page** (plus a runner-focused About): play, star/dislike, and scrobble, but no access to the library, settings, or downloads (enforced server-side, default-deny). Sign in with the shared **Guest login** (`RUN_PASSWORD`) **or a named player user** (optional username field) created in **Settings → Player access → Player users** — each player user is **always scoped to specific playlists** (run one, or all of them pooled via **All my music**; the server refuses the library/starred pool and any other playlist), while the shared Guest login is full-access. Reset/disable/delete a user and their sessions end at once; the Guest login stays a full-access shared login, so upgrades change nothing. Ideal for a shared phone/tablet or a dedicated running device; player logins stay signed in far longer (`RUN_SESSION_DAYS`, default 30). On desktop it keeps a slim sidebar with just **Run** + **About** (plus **Listen** when `PLAYER_LISTEN_MODE` — or a named user's per-user override — grants it, up to a Listen-only jukebox); on phones that collapses to a compact top bar with the same tabs, and Run uses a one-screen layout with the waveform/transport pinned to the bottom
- **Installable as an app (PWA)** — add it to your phone's home screen (requires HTTPS in front of the UI); lock-screen/headset media controls via the Media Session API; the app shell and API always stream live from your server (never cached)
- **Offline preloading** — the player fully downloads the next few queue tracks into a capped per-device cache (served by the service worker) so playback rides out network dead zones, and per-preset **Prepare offline** chips on the Run page download whole cadence queues in advance — Start run works even with the server unreachable. Automatic look-ahead depth, cache cap and per-preset size in Settings; needs HTTPS (streams as before without it)
...
**v2.14.0** — **Playlists learn set operations, and Run mode gets a memory.** **Compare** two playlists, **Merge** several into a Local one, or **Split** one by cadence or artist (identity-matched by file / ISRC / normalized artist+title; cadence splits use the exact run-queue rule) — all outputs are Local playlists, nothing writes back to Spotify or Navidrome. Every playlist detail opens with a **stats strip** (runtime, mini BPM histogram, plays, per-preset runnable counts), the library gains a sortable **Plays** column, every tempo run lands in a **run journal** with per-account run stats (admin / Guest / named player users), and each named player user can carry a **per-user Listen mode** override.

**v2.12.0** — **The queue follows your account across devices.** The player's saved state (queue, position in the track, shuffle/repeat, tempo lock, run/listen source, radio) is now snapshotted **server-side per account**: sign in on another device and the same queue is there, paused where you left off — one tap resumes. Last writer wins; a device that's actively playing keeps its own state, sign-out clears only that device (never the account's copy), volume stays per-device. Admin sessions share one snapshot, each named player user gets their own, and the shared Guest login stays per-browser.

**v2.11.3** — **Listen mobile polish.** The cover on the mobile Listen screen now absorbs all the leftover height (no more dead bands above the artwork and below the mode row — it stays a square, capped by the screen width), and the bottom **Playing/Queue** switcher stretches to the full content width as two equal, thumb-sized halves.

**v2.11.2** — **Listen fits one phone screen — and plays the whole library.** The Listen picker gains a **Whole library** source for full-access sessions (shelf order or shuffled; radio refills from it too — scoped player users still see only their playlists, enforced server-side). On phones, Listen now fits **one screen without scrolling**, like Run: the cover flexes to the leftover height, the transport pins low, and a **bottom Playing/Queue switcher** swaps the cover for the queue. Also fixes the kiosk top bar growing past its budget beside three tabs (which made kiosk pages scroll slightly on narrow phones).
