...
- **Run mode** — a full-screen tempo-run player that fits one phone screen: viewport-scaled cover art, big target-BPM readout with the tempo-lock toggle and a `native · stretch × octave → result` breakdown, ±1/±5 steps, four **named presets** or an in-place **queue view**, lyrics drawer, waveform + large transport; a **source picker** builds the queue from your whole library or a specific **playlist** (with an "N of M available" count); auto-queues octave-folded BPM matches (**starred tracks** first, **disliked tracks** never), **refills the queue automatically when the last track starts** favoring unplayed matches, and **locks the tempo** so every song stretches onto your step, pitch preserved; the desktop track-info column also shows **play count** and **file audio quality** (format + bit depth / sample rate, or bitrate)
