...
**v2.6.11** — **Play counts, stats & resilience.** Plays are now tallied **locally** by the built-in player, so play counts work **without Navidrome** and persist while it's disconnected; when scrobbling is on, plays are still forwarded and a pull **merges with MAX** instead of overwriting (never loses local plays, never double-counts). New Stats **Most played** card (top artists/tracks by play count, each linking to its artist/album/track page) plus an **all-time grabbed** counter that survives clearing the queue. Download-queue cleanup: **Delete** failed items and **Clear completed** — and "already have it?" now matches a grabbed file by its stamped Spotify id, so clearing can never trigger a re-download. **Slow-connection visibility**: a Buffering · N% / Offline readout on the player bar and Run page, with Run mode **pre-buffering the next tracks**. Mobile: honours the device **safe area** (player bar clears the home indicator), the **album/artist header** no longer squishes with its image hidden, and the **Library toolbar** collapses advanced filters behind a **Filters** button on phones.
