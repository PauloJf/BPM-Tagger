...
| `RUN_PASSWORD` | _(empty)_ | Optional **shared, full-access** password for a locked-down **player mode** (Run page + a runner-focused About only). For per-person accounts with playlist scoping, create **named run users** in Settings → Player Access → Run users. Also settable in Settings → Player Access |
...
- **Related & artist explorer** — a collapsible **Related · powered by Deezer** panel on every artist/album/track page (similar artists + tracks, fetched on expand); click any artist for a popup with a short bio, top tracks and full discography (albums + singles/EPs), adding single tracks or a whole album to the download queue. **30-second previews** (▶) on suggested/related rows — and on **inbox candidate cards** (Deezer candidates) so you can listen before choosing — play through the ducking player; yt-dlp candidates link to their source page instead. Read-only with the grabber off
...
- **Player mode** _(optional)_ — a locked-down view showing **only the Run page** (plus a runner-focused About): play, star/dislike, and scrobble, but no access to the library, settings, or downloads (enforced server-side, default-deny). Sign in with the shared `RUN_PASSWORD` **or a named run user** (optional username field) created in **Settings → Player Access → Run users** — each user is either full-access or **scoped to specific playlists** (a restricted user's run sources are filtered, and the server refuses the library/starred pool and any other playlist). Reset/disable/delete a user and their sessions end at once; the shared password stays a full-access guest, so upgrades change nothing. Ideal for a shared phone/tablet or a dedicated running device; player logins stay signed in far longer (`RUN_SESSION_DAYS`, default 30). On desktop it keeps a slim sidebar with just **Run** + **About**; on phones that collapses to a compact top bar with the same tabs, and Run uses a one-screen layout with the waveform/transport pinned to the bottom
...
**v2.6.11** — **Play counts, stats & resilience.** Plays are now tallied **locally** by the built-in player, so play counts work **without Navidrome** and persist while it's disconnected; when scrobbling is on, plays are still forwarded and a pull **merges with MAX** instead of overwriting (never loses local plays, never double-counts). New Stats **Most played** card (top artists/tracks by play count, each linking to its artist/album/track page) plus an **all-time grabbed** counter that survives clearing the queue. Download-queue cleanup: **Delete** failed items and **Clear completed** — and "already have it?" now matches a grabbed file by its stamped Spotify id, so clearing can never trigger a re-download. **Slow-connection visibility & smoothing**: a Buffering · N% / Offline readout on the player bar and Run page, an **adaptive rebuffer hold** (waits for a growing buffer before resuming after a stall — fewer, cleaner pauses), and Run mode **fully downloading the next tracks** so they play from local memory with no boundary stalls. Mobile: honours the device **safe area** (player bar clears the home indicator), the **album/artist header** no longer squishes with its image hidden, and the **Library toolbar** collapses advanced filters behind a **Filters** button on phones.

**v2.6.10** — **Run & mobile fixes + hardening.** The tempo-lock toggle now matches the **restored** lock after a reload; **star toggles work on auto-refilled** queue tracks; changing the **run source** prompts a Rebuild; and library top-ups in a playlist run are **dimmed** (not just italic). Mobile: the run-source picker moved **off the cover** into the Queue view, the Run queue no longer overflows the screen, and the **nav menu scrolls** so Logout/Support are reachable instead of hidden behind the player bar. Under the hood: server-side image fetches don't follow redirects (**SSRF** hardening), ffmpeg transcodes **time out**, the waveform cache is lock-guarded, and the database **enforces foreign keys** with orphan cleanup. Plus a forced-`Secure`-cookie option (`UI_FORCE_SECURE_COOKIE`) for TLS-terminating proxies, admin-only `/healthz` stats, a React **error boundary**, and **dependency scanning** in CI. And a one-time **"What's new"** popup after each update (admin only) with an in-app changelog view.
