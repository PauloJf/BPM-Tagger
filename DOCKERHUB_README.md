...
**v2.11.1** — **Listen: the regular music player — and optionally a jukebox kiosk.** A new **Listen** page (`/listen`) is the non-cadence player: full-screen now-playing (cover art, seekable waveform, star/dislike, shuffle/repeat/volume, the drag-to-reorder queue), playing any playlist in order or shuffled at native speed — **no BPM required**, un-analyzed tracks play too. A **radio** toggle keeps the queue refilling from the same playlist as it nears its end (recent tracks avoided, small pools recycled, dislikes never picked). And a new `PLAYER_LISTEN_MODE` setting (Settings → Player access) decides what **player logins** get besides Run: `off` (unchanged Run-only kiosk, the default), `on` (a Listen tab), `default` (Listen is the kiosk landing page), or `only` (a **pure jukebox** — Listen replaces Run) — enforced server-side, not just hidden. Suggestion rows across the app also gain **full-track previews** for songs you own (instead of the 30-second clip), and the in-app changelog renders sub-headings, italics and nested lists properly.
