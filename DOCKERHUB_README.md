...
- **Artists & Albums browse views** — a Tracks | Artists | Albums switcher with filterable card grids (track counts, years, average BPM) linking into the per-artist/per-album pages; the Tracks table carries a sortable **Plays** column (local play counts), and **Play all** follows the sort on screen
...
**v2.11.1** — **Listen: the regular music player — and optionally a jukebox kiosk.** A new **Listen** page (`/listen`) is the non-cadence player: full-screen now-playing (cover, seekable waveform, star/dislike, shuffle/repeat/volume, drag-to-reorder queue), playing any playlist in order or shuffled at native speed — **no BPM required**. A **radio** toggle keeps the queue refilling from the same playlist (recent tracks avoided, dislikes never picked). And `PLAYER_LISTEN_MODE` decides what **player logins** get besides Run: `off` (Run-only kiosk, the default), `on` (a Listen tab), `default` (Listen is the kiosk landing page), or `only` (a **pure jukebox** — no Run) — enforced server-side, not just hidden. Suggestion rows also gain **full-track previews** for songs you own (not just the 30-second clip).
