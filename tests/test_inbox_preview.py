"""Inbox candidate preview endpoint.

Lazy Deezer preview-URL resolution with a module-level TTL cache. All Deezer
calls are monkeypatched — no network. The module cache is cleared between tests
by the fixture so a hit/miss counter test is deterministic.
"""
...
@pytest.fixture
def preview(app):
    """Authenticated client + state, with the preview cache cleared."""
