...
- **Optional Subsonic API** (`web/subsonic/`, `SUBSONIC_ENABLED`): a stateless `/rest/*` blueprint registered only when enabled (CSRF-exempt, no session). Own credentials (`subsonic_credentials` table: hashed API key + generated password), admin-only in Phase 1. Queries in `db/subsonic.py`. Plan: `docs/plans/subsonic-api.md`.
